...
export default function ChatPanel({
  ...
  onSendMessage,
  autocompleteMatches,
  whisperReady, whisperSettingUp, whisperProgress,
  onWhisperSetup,
  showToast,
}) {
  ...
  const startRecording = useCallback(async () => {
    // Always request mic permission first — even if Whisper isn't ready yet.
    // This ensures the user sees the permission prompt on first click rather
    // than after the ~150MB Whisper download completes.
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      // Got permission — stop the test stream immediately
      stream.getTracks().forEach(t => t.stop());
    } catch(e) {
      const msg = e.name === 'NotAllowedError' ? 'Microphone permission denied — check Windows Settings → Privacy → Microphone' :
                  e.name === 'NotFoundError'   ? 'No microphone found — plug one in and try again' :
                  e.name === 'NotReadableError' ? 'Microphone is in use by another app' :
                  `Mic error: ${e.message}`;
      showToast(msg, 'error');
      return;
    }
    ...
    try {
      ...
      const msg = e.name === 'NotAllowedError'  ? 'Microphone permission denied — check Windows privacy settings' :
                  e.name === 'NotFoundError'     ? 'No microphone found — plug one in and try again' :
                  e.name === 'NotReadableError'  ? 'Microphone is in use by another app' :
                  `Mic error: ${e.message}`;
      showToast(msg, 'error');
    }
  }, [whisperReady, onWhisperSetup, showToast]);
  ...
  const micTitle = {
    'setting-up': 'Setting up voice — please wait…',
    listening:    'Click to stop recording',
    transcribing: 'Transcribing…',
    idle:         whisperReady ? 'Click to speak' : 'Click to set up voice (one-time ~150MB download)',
  }[micState];
  ...
  return (
    <div id="panel-chat">
      ...
      <div id="input-bar">
        ...
        {whisperSettingUp && whisperProgress && (
          <div className="warming-banner" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
              <span className="warming-spinner" />
              <span style={{ flex: 1 }}>🎙️ {whisperProgress.msg}</span>
              <span style={{ opacity: 0.6 }}>{whisperProgress.pct}%</span>
            </div>
            <div style={{ width: '100%', height: 3, background: 'rgba(0,212,255,0.15)', borderRadius: 2 }}>
              <div style={{ width: `${whisperProgress.pct}%`, height: '100%', background: 'var(--cyan)', borderRadius: 2, transition: 'width 0.3s ease' }} />
            </div>
          </div>
        )}
