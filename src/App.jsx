...
function useWhisper(showToast) {
  ...
  const [whisperProgress,   setWhisperProgress]   = useState(null); // null | { msg, pct }
  ...
  const setupWhisper = useCallback(async () => {
    ...
    setWhisperProgress({ msg: 'Starting…', pct: 0 });
    window.aria.onWhisperProgress(({ msg, pct }) => {
      setWhisperProgress({ msg, pct });
    });
  ...
  return { whisperReady, whisperSettingUp, whisperProgress, setupWhisper };
}
...
export default function App() {
  ...
  const { whisperReady, whisperSettingUp, whisperProgress, setupWhisper } = useWhisper(showToast);
